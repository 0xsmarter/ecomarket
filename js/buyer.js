...
function loadBuyerStats() {
  ...
  if (statsElements.totalOrders)
    statsElements.totalOrders.textContent = totalOrders;
  if (statsElements.cartItems) statsElements.cartItems.textContent = cartItems;
  if (statsElements.wishlistItems)
    statsElements.wishlistItems.textContent = wishlistItems;
  if (statsElements.totalSpent)
    statsElements.totalSpent.textContent = formatCurrency(totalSpent);
}
...
function loadFavoriteFarmers() {
  ...
  farmersGrid.innerHTML = favorites
    .map(
      (farmer) => `
